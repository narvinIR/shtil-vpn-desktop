...
`rustfmt` (через `rustup`, прав не нужно). **Сколько тестов — не пишем здесь
числом:** оно росло трижды за август, а записанное руками протухает молча. Живое
берётся прогоном выше, снимок — в `../../product-facts.json` (`counts.desktop_tests`,
`counts.desktop_script_tests`); в старт сессии он подаёт только версии, за числом
тестов идти туда руками. Маркер `<!--f:…-->` здесь не поставить: сверщик этот
файл не видит вовсе (`DOC_ROOTS` в `scripts/platforms/checks.py`), а расширить
его на `desktop/app` — значит потащить 409 файлов `node_modules`. Прежняя
запись «на macOS собирается меньше, эталон — Linux» **опровергнута замером
16.08.2026**: Мак дал ровно столько же, сколько облако.
