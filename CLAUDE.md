...
`rustfmt` (через `rustup`, прав не нужно). Живой прогон даёт 206 тестов Rust и
16 тестов сборочных скриптов. **Число зависит от системы:** на macOS часть тестов
не собирается (там 200), эталон — Linux, потому что на нём же гоняет облако.
